...
Tester concurrence/rollback/redémarrage lorsqu'ils sont touchés. Les tests de persistance utilisent PostgreSQL/Testcontainers via le profil integration. Dans le POM du dépôt, -Pintegration verify exécute aussi le cycle standard : ne pas ajouter un verify séparé après sa réussite sans changement, diagnostic ou exigence applicable. Une analyse des tests existants donne une couverture documentée, pas un résultat d'exécution nouveau.
