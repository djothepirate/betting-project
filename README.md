...
- MVP-001 : `ACTIVE - IMPLEMENTATION`, lots 0 à 2 terminés, lot 3 prochain mais `PENDING`, **5 critères sur 20** démontrés. Le lot 2 livre le [budget durable](docs/contracts/provider-budget-v1.md), les réservations atomiques, la cadence distincte et les incidents fournisseur avec V009. Qualification locale : **413 tests standards et 157 tests PostgreSQL/Testcontainers**, sans échec ni omission ; voir le [rapport d'exécution](docs/work-orders/MVP-001-lot2-execution.md). Le registre réel reste vide ; aucun appel HTTP, endpoint ou worker n'est introduit par ce lot. La branche `feature/V0.1.0-RC01-CODEX-MVP-001` sera livrée par PR vers `feature/V0.1.0-RC01` après ses lots restants ; chaque lot terminé est commité, poussé et contrôlé par CI sur son SHA. Les publications des lots 0 et 1 restent des preuves historiques distinctes dans le [Work Order](docs/work-orders/MVP-001.md) ;
