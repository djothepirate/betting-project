...
- MVP-001 : `ACTIVE - IMPLEMENTATION`, lots 0 à 3 terminés, lot 4 prochain mais `PENDING`, **9 critères sur 20** démontrés. Le lot 3 raccorde les [adaptateurs calendrier](docs/contracts/provider-calendar-collection-v1.md) au budget qualifié, avec audit, preuves natives/dérivées et replay hors réseau (V010). Qualification locale : **550 tests standards et 196 tests PostgreSQL/Testcontainers**, sans échec ni omission ; voir le [rapport d'exécution](docs/work-orders/MVP-001-lot3-execution.md) et le [runbook local](docs/runbooks/provider-calendar-local.md). Registre réel vide, clients désactivés, aucun appel fournisseur réel, endpoint ou worker ajouté. La branche `feature/V0.1.0-RC01-CODEX-MVP-001` sera livrée par PR vers `feature/V0.1.0-RC01` après ses lots restants ; chaque lot terminé est commité, poussé et contrôlé par CI sur son SHA. Les preuves des lots précédents restent historiques dans le [Work Order](docs/work-orders/MVP-001.md) ;
