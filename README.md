...
- MVP-001 : `ACTIVE - IMPLEMENTATION`, lots 0 à 4 terminés localement, lot 5 prochain mais `PENDING`, **12 critères sur 20** démontrés. Le lot 4 ajoute les [jobs de collecte](docs/contracts/collection-jobs-v1.md), claims atomiques, baux, reprise et planification datée explicite (V011), raccordés au budget et aux adaptateurs existants. Qualification locale : **572 tests standards et 224 tests PostgreSQL/Testcontainers**, sans échec ni omission ; voir le [rapport d'exécution](docs/work-orders/MVP-001-lot4-execution.md) et le [runbook local](docs/runbooks/collection-jobs-local.md). Registre réel vide, clients et boucle worker désactivés, aucun appel fournisseur réel ni endpoint ajouté. La CI GitHub du lot 3 n'a pas démarré pour raison de facturation ; les résultats locaux ne valent pas qualification CI. La branche `feature/V0.1.0-RC01-CODEX-MVP-001` sera livrée par PR vers `feature/V0.1.0-RC01` après ses lots restants ; chaque lot terminé est commité, poussé et contrôlé par CI sur son SHA. Les preuves antérieures restent historiques dans le [Work Order](docs/work-orders/MVP-001.md) ;
