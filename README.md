...
- CAT-002 : la Pull Request `#8` reste ouverte sur `codex/cat-002-catalog`. Le correctif fonctionnel `6fb69e2` distingue les caractères fournisseur littéraux `*`, `?` et `%` des jokers interdits dans les affectations de configuration ; il est publié, couvert par une seconde revue humaine, validé à 36 critères sur 36 et par quatre checks Windows/Linux verts, dont PostgreSQL/Testcontainers sous Linux. La discussion P2 est résolue. Le présent alignement documentaire ne change aucun comportement ; la fusion reste conditionnée aux checks du HEAD courant, à la revue finale et à l'absence de nouvelle remarque ou de conflit ;
